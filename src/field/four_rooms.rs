    ...
    for room in 0..4 {
        ...
        for i in 0..size.x {
            for j in 0..size.y {
                ...
                if i > 0 {
                    neighbors.push(((i - 1) * size.y + j) as usize + offset);
                }
                if i < size.x - 1 {
                    neighbors.push(((i + 1) * size.y + j) as usize + offset);
                }
                if j > 0 {
                    neighbors.push((i * size.y + j - 1) as usize + offset);
                }
                if j < size.y - 1 {
                    neighbors.push((i * size.y + j + 1) as usize + offset);
                }
    ...
    match start_points_type {
        1 => {
            start_cells.push(vec![
                (size.x * size.y * 0 + 0) as usize,
                (size.x * size.y * 0 + size.y * (size.x - 1)) as usize,
                (size.x * size.y * 1 + 0) as usize,
                (size.x * size.y * 1 + size.y * (size.x - 1)) as usize,
                (size.x * size.y * 2 + 0) as usize,
                (size.x * size.y * 2 + size.y * (size.x - 1)) as usize,
                (size.x * size.y * 3 + 0) as usize,
                (size.x * size.y * 3 + size.y * (size.x - 1)) as usize,
            ]);
            start_cells.push(vec![
                (size.x * size.y * 0 + (size.y - 1)) as usize,
                (size.x * size.y * 0 + size.x * size.y - 1) as usize,
                (size.x * size.y * 1 + (size.y - 1)) as usize,
                (size.x * size.y * 1 + size.x * size.y - 1) as usize,
                (size.x * size.y * 2 + (size.y - 1)) as usize,
                (size.x * size.y * 2 + size.x * size.y - 1) as usize,
                (size.x * size.y * 3 + (size.y - 1)) as usize,
                (size.x * size.y * 3 + size.x * size.y - 1) as usize,
            ]);
        }
        2 => {
            start_cells.push(vec![
